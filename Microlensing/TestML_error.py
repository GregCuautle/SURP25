...
def trueSolution(mlens, zlens_list, xs, ys, z, cal_ang=True):
    zlens = [complex(p[0], p[1]) for p in zlens_list]
    z = complex(z[0], z[1])
    zs = complex(xs, ys)
    ...
    # --- residual of lens equation ---
    dzs = zs - z
    for i in range(len(mlens)):
        dzs += (mlens[i] / conj(z - zlens[i]))
    tol = tol_abs + tol_rel * max(1.0, abs(zs))

    flag = 0
    mu = -1.0e10
    lambda1 = lambda2 = thetaJ = 0.0

    if abs(dzs) < tol:
        # Jacobian and magnification
        x, y = z.real, z.imag
        Jxx, Jxy = 1.0, 0.0
        for i in range(len(mlens)):
            dx = x - zlens[i].real
            dy = y - zlens[i].imag
            r2 = dx*dx + dy*dy + TINY
            Jxx += mlens[i]*(dx*dx - dy*dy)/(r2*r2)
            Jxy += 2.0*mlens[i]*dx*dy/(r2*r2)
        Jyy, Jyx = 2.0 - Jxx, Jxy
        detJ = Jxx*Jyy - Jxy*Jyx
        mu = 1.0 / detJ

        # reject unphysical or divergent solutions
        if np.isfinite(mu) and abs(mu) < 1e8:
            flag = 1
            if cal_ang:
                sum2 = (Jxx + Jyy)/2.0
                sq = math.sqrt(max(0.0, sum2*sum2 - 1.0/mu))
                lambda1 = sum2 + sq
                lambda2 = sum2 - sq
                thetaJ  = 0.5*math.atan2(2.0*Jxy, (Jyy - Jxx) + TINY)
...
def get_allimgs_with_mu(mlens, z, xsCenter, ysCenter, rs, NLENS, Phis):
    ...
    R_MAX, MU_MAX = 3.0, 1e1

    for phi in Phis:
        xs = xsCenter + math.cos(phi)*rs
        ys = ysCenter + math.sin(phi)*rs
        XS.append(xs); YS.append(ys)

        res = sol_len_equ_cpp(mlens, z, xs, ys, NLENS)
        for i in range(DEGREE):
            ok, mu, *_ = trueSolution(mlens, z, xs, ys, res[i], cal_ang=False)
            x_i, y_i = res[i][0], res[i][1]
            r_i = (x_i*x_i + y_i*y_i)**0.5

            if ok and np.isfinite(mu) and abs(mu) < MU_MAX and r_i < R_MAX:
                imgXS.append(x_i); imgYS.append(y_i); imgMUs.append(abs(mu))
            else:
                falseimgXS.append(x_i); falseimgYS.append(y_i)
