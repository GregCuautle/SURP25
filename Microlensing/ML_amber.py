...
class TwoLens1S:
    def __init__(self, t0, tE, rho, u0_list, q, s, alpha_deg,
                 t_lc=None, a1=0.0, num_points=1000):
        import VBMicrolensing
        self.t0 = float(t0)
        self.tE = float(tE)
        self.rho = float(rho)
        ...
        self.tau = np.linspace(-4, 4, self.num_points)
        self.t   = self.t0 + self.tau * self.tE
        self.highres_tau = np.linspace(-4, 4, self.num_points)
        ...
        self.VBM.a1 = self.a1
        # self.VBM.SetMethod(self.VBM.Method.Nopoly)  # keep default unless you need it
    ...
    def _centroid_at(self, y1, y2):
        ...
        # Linear LD via ring differencing
        ...
        for k in range(1, len(radii)):
            ...
            I    = 1.0 - self.a1 * (1.0 - mu)
            Nx += I * dSAx
            Ny += I * dSAy
            D  += I * dSA
        return (Nx/D, Ny/D) if D > 0 else (np.nan, np.nan)

    def _vb_path(self, tau):
        # VB-consistent analytic path (matches your advisor’s convention)
        y1 = self._u0*np.sin(self.alpha_rad) - tau*np.cos(self.alpha_rad)
        y2 = -self._u0*np.cos(self.alpha_rad) - tau*np.sin(self.alpha_rad)
        return y1, y2

    def _prepare_systems(self):
        systems = []
        for u0, color in zip(self.u0_list, self.colors):
            self._u0 = float(u0)  # used by _vb_path
            y1s,  y2s  = self._vb_path(self.tau)
            y1s_hr, y2s_hr = self._vb_path(self.highres_tau)
            y1s_lc, y2s_lc = self._vb_path(self.tau_lc)

            # Centroids (standard + high-res + light-curve grids)
            cent_x    = [self._centroid_at(x, y)[0] for x, y in zip(y1s,    y2s)]
            cent_y    = [self._centroid_at(x, y)[1] for x, y in zip(y1s,    y2s)]
            cent_x_hr = [self._centroid_at(x, y)[0] for x, y in zip(y1s_hr, y2s_hr)]
            cent_y_hr = [self._centroid_at(x, y)[1] for x, y in zip(y1s_hr, y2s_hr)]
            cent_x_lc = [self._centroid_at(x, y)[0] for x, y in zip(y1s_lc, y2s_lc)]
            cent_y_lc = [self._centroid_at(x, y)[1] for x, y in zip(y1s_lc, y2s_lc)]

            # Photometry on t_lc grid (radians + logs like 3L1S)
            mag, *_ = self.VBM.BinaryLightCurve(
                [np.log(self.s), np.log(self.q), self._u0, self.alpha_rad,
    ...
    def plot_caustic_critical_curves(self):
        ...
        for cau in caustics: plt.plot(cau[0], cau[1], 'r', lw=1.2)
        for crit in criticalcurves: plt.plot(crit[0], crit[1], 'k--', lw=0.8)

        # lens positions (VBM convention)
        x1 = -self.s * self.q / (1 + self.q)
        x2 =  self.s / (1 + self.q)
        plt.plot([x1, x2], [0, 0], 'ko')

        for sys in self.systems:
            plt.plot(sys['y1s'], sys['y2s'], '--', color=sys['color'], alpha=0.6)

        plt.xlim(-2, 2); plt.ylim(-2, 2)
        plt.xlabel(r"$\theta_x$ ($\theta_E$)"); plt.ylabel(r"$\theta_y$ ($\theta_E$)")
        plt.title("2L1S Lensing Event")
        plt.gca().set_aspect("equal"); plt.grid(True)
        plt.legend(handles=[lens_handle, caustic_handle, crit_curve_handle, q_handle, s_handle],
    ...
    def plot_light_curve(self, trange=None, max_points=400):
        ...
        if trange is not None:
            ...
            tau = self.tau_lc; t_arr = self.t_lc

        for sys in self.systems:
            # recompute mag on chosen grid to mirror 3L1S
            mag, *_ = self.VBM.BinaryLightCurve(
                [np.log(self.s), np.log(self.q), sys['u0'], self.alpha_rad,
                 np.log(self.rho), np.log(self.tE), self.t0],
                t_arr
            )
            plt.plot(tau, mag, color=sys['color'], label=fr"$u_0$ = {sys['u0']}")
        plt.xlabel(r"$\tau$"); plt.ylabel("Magnification")
        plt.title("Binary Lens Light Curve (VBM)")
        if trange is not None: plt.xlim(trange)
        plt.grid(True); plt.legend(); plt.tight_layout(); plt.show()

    def plot_centroid_trajectory(self):
        plt.figure(figsize=(6, 6))
        for sys in self.systems:
            dx = sys['cent_x_hr'] - sys['y1s_hr']
            dy = sys['cent_y_hr'] - sys['y2s_hr']
            plt.plot(dx, dy, color=sys['color'], label=fr"$u_0$ = {sys['u0']}")
        plt.xlabel(r"$\theta_x$ ($\theta_E$)")
        plt.ylabel(r"$\theta_y$ ($\theta_E$)")
        plt.title("Centroid Shift Trajectories")
        plt.gca().set_aspect("equal"); plt.legend(); plt.grid(True); plt.tight_layout(); plt.show()
    ...
    def plot_centroid_shift(self):
        ...
        for sys in self.systems:
            ...
            plt.plot(self.highres_tau, dtheta, color=sys['color'], label=fr"$u_0$ = {sys['u0']}")
        plt.xlabel(r"$\tau$"); plt.ylabel(r"$|\delta \vec{\Theta}|$")
...
class ThreeLens1S:
    def __init__(self, t0, tE, rho, u0_list, q2, q3, s2, s3, alpha_deg, psi_deg,
        ...
        self.highres_tau = self.tau
        self.highres_t   = self.t
